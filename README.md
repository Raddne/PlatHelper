...
Automatic inventory reads the login token from the running game's memory. With Steam's Proton container this works out of the box. If PlatHelper reports "WF unreachable from sandbox" (PlatHelper started through `appimage-run`, `steam-run` or Flatpak, usual on NixOS) or "WF memory blocked (ptrace_scope)", see [game memory access](docs/features/getting-started.md#if-the-inventory-does-not-load-game-memory-access) for the fix: start PlatHelper outside the sandbox, or relax `kernel.yama.ptrace_scope`.
